        ...
        task::spawn_blocking(move || {
            {
                ...
                let _x = self_clone.checkpoint_in_progress.blocking_lock();
            }
            ...
            if let Ok(mut conn) = self_clone.read_pool.get() {
                {
                    let pool_state = self_clone.read_pool.state();
                    metrics
                        .db_connections
                ...
                for filter in sub.filters.iter() {
                    ...
                    while let Some(row) = event_rows.next()? {
                        ...
                        if row_count % 100 == 0 {
                            {
                                if self_clone.checkpoint_in_progress.try_lock().is_err() {
